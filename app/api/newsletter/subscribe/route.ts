...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existing && existing.length > 0) {
      ...
      await sendEmail({
        to: email,
        subject: "Welcome back to the ClothCraft newsletter!",
        html: `
    ...
    await sendEmail({
      to: email,
      subject: "Welcome to the ClothCraft newsletter!",
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto;">
          <h1 style="color: #333;">Welcome to the ClothCraft newsletter!</h1>
